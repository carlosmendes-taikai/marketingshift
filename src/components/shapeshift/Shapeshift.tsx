...
export function Shapeshift() {
  ...
  // Long or multi-line text (a pasted post) switches to a smaller font; the box grows with its content.
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === "/" && text === "") {
      ...
      // Enter saves; Shift+Enter adds a line (pasted drafts keep their line breaks).
      e.preventDefault();
        ...
        <MorphContainer readiness={readiness} edge={ghost ? null : (meta?.edge ?? null)}>
          <motion.div layout="position" className="relative flex min-h-[72px] items-start px-5 py-5">
            <textarea
              ...
              enterKeyHint="done"
              className={cn(
                "relative z-[1] block max-h-[40vh] w-full resize-none overflow-y-auto bg-transparent pe-6 font-[450] tracking-[-0.01em] text-foreground caret-brand outline-none",
                long ? "text-[15px] leading-6" : "text-[22px] leading-8",
              )}
            />
            ...
            <span
              aria-hidden
              className={cn(
                "absolute end-5 top-[33px] size-1.5 rounded-full bg-brand transition-opacity duration-300 ease-out",
